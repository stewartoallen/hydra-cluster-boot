...
var http = {
    ...
    template : function(res, templateName, values) {
        config.get(['template',templateName], function(err, data) {
            if (!err) {
                var render = swig.render(data, {locals:values});
                http.ok(res, render);
            } else {
                http.fail(res, err, 500);
            }
        });
    },
...
var render = {
    boot : function(req, res, url) {
        ...
        config.getJS(['cluster', q.cluster], function(err, cluster) {
            if (err) return http.fail(res, 'echo invalid cluster '+ q.cluster);
            config.getJS(['cluster', q.cluster, 'host', hostname], function(err, host) {
                if (err) return http.fail(res, 'echo invalid host '+ hostname);
...
var api = {
    ...
    boot_node : function(query, remote, callback) {
        ...
        if (query.cluster && query.process) {
            ...
            config.getJS(['host',remote], function(err, val) {
    ...
    init : function(port) {
        ...
        swig.setDefaults({loader:{
            ...
            load : function(ident, callback) {
                config.get(['template',ident], callback);
            }
        }});
