...
var get = function(arr, callback) {
    db.get(key(arr), callback);
};

var put = function(arr, val, callback) {
    db.put(key(arr), val, callback);
};
    ...
    dump : function(dir) {
        ...
        db.createReadStream()
            .on('data', function(data) {
                var key = data.key;
                var val = data.value;
                fs.writeFileSync([dir,key].join('/'), JSON.stringify(val));
            })
            .on('end', function() {
                db.close();
            });
